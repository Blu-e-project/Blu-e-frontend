...
class RequestMentoringActivity : AppCompatActivity() {
    ...
    private var infoPost: ArrayList<Pick>? = null
    private var infoPostUserId: Int = 0
    private var userId = MainApplication.prefs.getString("userId", "")
    private var role = MainApplication.prefs.getString("role", "")  //멘토1 멘티2
...
//        adapter.notifyItemChanged(commentList.size)
        ...
        if (isMentor == 2) {
            //<           멘티면 ->                멘토>  //(글쓴이(토)/댓쓴이(티)/일반회원(잠재적 댓쓴이)(티))
            api.requestAPostOfMentee(4).enqueue(object : Callback<PickResponse> {
            ...
            api.requestMenteeComments(4).enqueue(object : Callback<PickCommentResponse> {
                override fun onResponse(
            ...
            //<로그인 유저가 멘토면 -> 글쓴 사람은 반드시 멘티> //(글쓴이(티)/댓쓴이(토)/일반회원(잠재적 댓쓴이)(토))
            api.requestAPostOfMentor(4).enqueue(object : Callback<PickResponse> {
                @RequiresApi(Build.VERSION_CODES.O)
                override fun onResponse(
                    call: Call<PickResponse>,
                    response: Response<PickResponse>
                    ...
                    if (body != null) {
                        if (body.code == 1000) {
                            ...
                            viewBinding.create.text = infoPost!!.get(0).createdAt
                            viewBinding.create.text = infoPost!!.get(0).updatedAt
...
//                            viewBinding.update.text = convertTimeToDate(infoPost.get(0).updatedAt)
            ...
            //retrofit (get 댓글)
            api.requestMentorComments(4).enqueue(object: Callback<PickCommentResponse> {
                        ...
                        if(body.code == 1000) {
                            ...
                            if(matchingStatus == 1) { //모집중
                                if(infoPostUserId == loginUserId) {
                                    ...
                                    //+댓쓴이(멘토)라면 //+adapter 체크 (댓쓴이가 로그인유저인지)
                                    //adapter.updateCompletdTv(0)
                                    //adapter.updateAcceptBtnv(0)
                                    //adapter.updateCommentMenuv(1)
                                    //disappearCommentForm()
...
//        val getTime = sdf.format(date)
        ...
        //+ 로그인 유저가 멘티 회원임.
        if(role.toInt() == 1) { //아 이거 왜 바꿔야지 됌?...... 2로 생각했었음.
            Log.d("여기로 들어왔을과?", "묭!")
            api.commentWritingAsMentee(2, commentContent).enqueue(object: Callback<ResponseData> {
                override fun onResponse(call: Call<ResponseData>, response: Response<ResponseData>) {
                    val body = response.body()?: return
                    if(body != null) {
                        if(body.code == 1000) {
            ...
            //+ 로그인 유저가 멘토회원임.
            Log.d("여기로 들어왔을과?", "믱!")
            api.commentWritingAsMentor(4, commentContent).enqueue(object: Callback<ResponseData> {
                override fun onResponse(call: Call<ResponseData>, response: Response<ResponseData>) {
                    val body = response.body()?: return
                    if(body != null) {
                        if(body.code == 1000) {
